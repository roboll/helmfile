...
RUN set -x && \
    wget --retry-connrefused "${HELM_LOCATION}/${HELM_FILENAME}" && \
...
RUN set -x && \
    wget --retry-connrefused "https://storage.googleapis.com/kubernetes-release/release/${KUBECTL_VERSION}/bin/linux/amd64/kubectl" && \
...
RUN set -x && \
    wget --retry-connrefused "https://github.com/kubernetes-sigs/kustomize/releases/download/kustomize/${KUSTOMIZE_VERSION}/kustomize_${KUSTOMIZE_VERSION}_linux_amd64.tar.gz" && \
